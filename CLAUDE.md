...
SPデザインは **750px キャンバス** での値。  
768px のブレイクポイントでデザインの値を `vw-sp()` で適用し、375px まで可変対応する。

```
vw-sp($size)  // $size は Figma の 750px 時の px 値
```

例：Figma で width: 480px の場合 → `width: vw-sp(480)`
